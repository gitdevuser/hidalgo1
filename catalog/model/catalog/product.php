...
class ModelCatalogProduct extends Model {
	public function updateViewed($product_id) {
        ...
        public function getCatProductPrice($product_id) {
            $query = $this->db->query("SELECT * FROM product_to_category as table1
        ...
        public function getCatProductName($product_id) {
            $query = $this->db->query("SELECT * FROM product_to_category as table1
                                LEFT JOIN category_description as table2 ON table1.category_id=table2.category_id
                                WHERE table1.product_id = '".$product_id."'");
            if ( $query->num_rows > 0 ) {
        ...
        public function getProductsStyleID($product_id) {
            ...
            $q_prodCat = $this->db->query("SELECT * FROM product_to_category WHERE product_id = '".$product_id."'");
            if ( $q_prodCat->num_rows > 0 ) {
                 $id = '';
                 foreach( $q_prodCat->rows as $rs1 ){ $id = $rs1['category_id']; }
                 return $id;
            } else {
                return false;
            }
        ...
        public function getProdbyCat($cat_id) {
            $query = $this->db->query("SELECT * FROM product_to_category as table1
                                LEFT JOIN product as table2 ON table1.product_id =table2.product_id
                                LEFT JOIN product_description as table3 ON table1.product_id=table3.product_id
                                WHERE table1.category_id = '".$cat_id."'");
            if ( $query->num_rows > 0 ) {
                 return $query->rows;
            } else {
                return false;
        ...
        public function getCatDescripcion($cat_id) {
            $query = $this->db->query("SELECT * FROM category_description WHERE category_id ='".$cat_id."'" );
            if ( $query->num_rows > 0 ) {
                 return $query->rows;
            } else {
                return false;
        ...
        public function getTallaSubtalla( $arr_talla, $tipo ) {
            ...
            if ( $tipo == 'subtalla' ) {
                $table = 'subtalla';
                $id_table = 'secsizes_id';
            }

            /* Ordenar unicos */
            $arrayUn = array();
            foreach ( $arr_talla as $unicos ) {
                $arrayUn[] = $unicos['talla'];
            }
            $arrayIDProduct = array_unique( $arrayUn );

            /* Ordenar orden sort() */
            sort( $arrayIDProduct );
            /* Ordenar anadir id_producto*/
            $arrayNombre = array();
            foreach ( $arrayIDProduct as $nombre ) {
                $query = $this->db->query("SELECT * FROM ".$table." WHERE ".$id_table." = '". $nombre."'" );
                if ( $query->num_rows > 0 ) {
                    if ( $tipo == 'talla' ) {
                         $arrayNombre[] = array( 'tid' =>$query->row['sizes_id'], 't_nom' => $query->row['ssize'] );
                    }
                    if ( $tipo == 'subtalla' ) {
                         $arrayNombre[] = array( 'tid' =>$query->row['secsizes_id'], 't_nom' => $query->row['ssize'] );
                    }
                }
            }
            $arrTallaList = array();
            foreach ( $arrayNombre as $Prodval ) {
                $product_id = '';
                $selected = '';
                foreach ( $arr_talla as $idprod_val ) {
                   if ( $idprod_val['talla'] == $Prodval['tid'] ) {
                        $product_id = $idprod_val['product_id'];
                        $selected = $idprod_val['selected'];
                   }
                }

                $arrTallaList[] = array( 'talla_id'     => $Prodval['tid'],
                                         'talla_nombre' => $Prodval['t_nom'],
        ...
        public function getTallaSubtallaNombre( $id, $tipo ) {
            if ( $tipo == 'talla' ) {
                 $table = 'tallas';
                 $id_table = 'sizes_id';
            }
            ...
            $query = $this->db->query("SELECT * FROM ".$table." WHERE ".$id_table." = '".$id."'" );
            if ( $query->num_rows > 0 ) {
                 return $query->row['ssize'];
            } else {
                 return false;
            }
        }
		...
		if ($query->num_rows) {
			return array(
				...
				'viewed'           => $query->row['viewed'],
                                'composicion'      => $query->row['composicion'],
                                'cuidado'      => $query->row['cuidado'],
                                'corte'      => $query->row['corte'],
                                'talla' => $query->row['talla'],
                                'subtalla' => $query->row['subtalla'],
				'color' => $query->row['color'],
                                'caracteristica' => $query->row['caracteristica'],
				'descuento' => $query->row['descuento']				
