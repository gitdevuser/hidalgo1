...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		if ($product_info) {
			...
			if ($product_info['caracteristica']) {
				$this->data['caract'] = $product_info['caracteristica'];
			} else {
				$this->data['caract'] = '';
			}

			if ($product_info['descuento']) {
				$this->data['descuento'] = $product_info['descuento'];
			} else {
				$this->data['descuento'] = '';
			}

                        /* Obtener productos relacionados a Categoria */
                        $catID = $this->model_catalog_product->getProductsStyleID($product_id);
                        $resultsStyle = '';
                        if ( $catID ) {
                             $resultsStyle = $this->model_catalog_product->getProdbyCat($catID);
                             $ColorsbyItems = array();
                             foreach ( $resultsStyle as $rel_item ) {
                                 $desc = html_entity_decode($rel_item['description'], ENT_QUOTES, 'UTF-8');
                                 if ( $rel_item['color'] ) {
                                      $Nwcolor = unserialize($rel_item['color']);
                                      $ColorsbyItems = $Nwcolor[0];
                                 }
                             }
                        }
