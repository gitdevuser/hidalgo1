...
class ModelCatalogProduct extends Model {
	...
	public function addProduct($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', 
                    ...
                    manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "',
                    sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), novedad = ".(int)$novedad );
		
		$product_id = $this->db->getLastId();

                $setTalla = '';
                if ( $data['talla'] !="" ) {
                     $setTalla = " SET talla = '" . $data['talla'] . "'";
                }
                $setSubtalla = '';
                $cm1 = '';
                if ( $data['subtalla'] !="" ) {
                     if( $setTalla !="" ){ $cm1 = ', '; }
                     if( $cm1 == "" ){ $cm1 = ' SET '; }
                     $setSubtalla = $cm1." subtalla = '" . $data['subtalla'] . "'";
                }
                $setColor = '';
                $cm2 = '';
                if ( $data['color'] !="" ) {
                ...
                if ( $data['color'] !="" ) {
                     ...
                     if( $setColor !="" ){ $cm3 = ', '; }
                     if( $cm3 == "" ){ $cm3 = ' SET '; }
                     $setnomColor = $cm3." nombre_color = '" . $data['nombre_color'] . "' ";
                }

                $this->db->query("UPDATE " . DB_PREFIX . "product ".$setTalla.$setSubtalla.$setColor.$setnomColor." WHERE product_id = '" . (int)$product_id . "'");
		...
		/* Actualizar Talla, subtalla, color, caracteristicas */
        public function updateTalla($product_id, $data){
                $n_talla = '';
                if( $data !="" ) {
                    $n_talla = $data;
                }
                $this->db->query("UPDATE " . DB_PREFIX . "product SET talla = '" . $n_talla . "'
                                                                  WHERE product_id = '" . (int)$product_id . "'");
        }

        public function updateSubtalla($product_id, $data){
                $n_talla = '';
                if( $data !="" ) {
                    $n_talla = $data;
                }
                $this->db->query("UPDATE " . DB_PREFIX . "product SET subtalla = '" . $n_talla . "'
                                                                  WHERE product_id = '" . (int)$product_id . "'");
        }

        public function updateColor($product_id, $data){
                $n_talla = '';
                if( $data !="" ) {
                    $n_talla = $data;
                }
                $this->db->query("UPDATE " . DB_PREFIX . "product SET color = '" . $n_talla . "'
                                                                  WHERE product_id = '" . (int)$product_id . "'");
        }

        public function updateNombreColor($product_id, $data){
                $n_color = '';
                if( $data !="" ) {
                    $n_color = $data;
                }
                $this->db->query("UPDATE " . DB_PREFIX . "product SET nombre_color = '" . $n_color . "'
                                                                  WHERE product_id = '" . (int)$product_id . "'");
        }

        public function updateCaract($product_id, $data){
                $n_caract = '';
                if( $data !="" ) {
                    $n_caract = $data;
                }
                $this->db->query("UPDATE " . DB_PREFIX . "product SET caracteristica = '" . $n_caract . "'
                                                                  WHERE product_id = '" . (int)$product_id . "'");
        }

        public function updateDesc($product_id, $data){
                $n_desc = '';
                if( $data !="" ) {
                    $n_desc = $data;
                }
                $this->db->query("UPDATE " . DB_PREFIX . "product SET descuento = '" . $n_desc . "'
                                                                  WHERE product_id = '" . (int)$product_id . "'");
        }
        ...
        public function getsSubTallas(){
                $query = $this->db->query("SELECT * FROM subtalla");
                if( $query->num_rows > 0 ) {
                    return $query->rows;
                } else {
                    return false;
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW(), composicion ='" .$this->db->escape($data['composicion']). "', cuidado = '" .$this->db->escape($data['cuidado']). "', corte = '" .$this->db->escape($data['corte']). "'  WHERE product_id = '" . (int)$product_id . "'");
