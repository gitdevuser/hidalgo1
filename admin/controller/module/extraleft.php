...
class ControllerModuleExtraleft extends Controller {
	...
	public function index() {   
		...
		$this->document->setTitle('Bloques Columna Izquierda');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {			
			...
			$this->session->data['success'] = $this->language->get('text_success');*/
                        $this->db->query( 'TRUNCATE TABLE `left_extra_module`' );

                        $this->request->post['titulo'];
                        $posicion = $this->request->post['posicion'];
                        $visible = $this->request->post['visible'];

                        $e = 0;
                        $t = 0;
                        
                        foreach( $this->request->post['titulo'] as $rs1 ) {
                            $t = $e++;
                            $visible_1 = '';
                            if ( isset( $visible[$t] ) && $visible[$t] != "" ) { $visible_1 = 1; }
                            $this->db->query( "INSERT INTO `left_extra_module` (`extra_nom`, `weight`, `visible` )
                                                 VALUES ('".$rs1."', '".$posicion[$t]."', '".$visible_1."')" );
                        }
		...
		$this->data['heading_title'] = 'Bloques Columna Izquierda';
		...
		$this->data['action'] = $this->url->link('module/extraleft', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->template = 'module/extraleft.tpl';
		$this->children = array(
			'common/header',
			'common/footer',
		);

                $extraLeft = $this->model_catalog_category->getExtraLeftModule();
                $this->data['extraLeft'] = '';
                if ( count ( $extraLeft ) > 0 ) { $this->data['extraLeft'] = $extraLeft; }
