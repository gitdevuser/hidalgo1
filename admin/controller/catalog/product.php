...
class ControllerCatalogProduct extends Controller {
	...
	$this->load->model('catalog/product');
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_model'])) {
				$url .= '&filter_model=' . $this->request->get['filter_model'];
                        ...
                        /* Actualizar Talla, subtalla, color , nombre color, caracteristicas, descuento */
                        if(isset( $this->request->post['talla'] )) {
                            $this->model_catalog_product->updateTalla($this->request->get['product_id'], $this->request->post['talla'] );
                        } else {
                            $this->model_catalog_product->updateTalla($this->request->get['product_id'], '' );
                        }
                        
                        if(isset( $this->request->post['subtalla'] )) {
                            $this->model_catalog_product->updateSubtalla($this->request->get['product_id'], $this->request->post['subtalla'] );
                        } else {
                            $this->model_catalog_product->updateSubtalla($this->request->get['product_id'], '' );
                        }
						
                        if(isset( $this->request->post['color'] )) {
                            $this->model_catalog_product->updateColor($this->request->get['product_id'], $this->request->post['color'] );
                        } else {
                            $this->model_catalog_product->updateColor($this->request->get['product_id'], '' );
                        }

                        if(isset( $this->request->post['nombre_color'] )) {
                            $this->model_catalog_product->updateNombreColor($this->request->get['product_id'], $this->request->post['nombre_color'] );
                        } else {
                            $this->model_catalog_product->updateNombreColor($this->request->get['product_id'], '' );
                        }
                        

                        if(isset( $this->request->post['caract'] )) {
                            $arrCaract = array();
                            foreach ( $this->request->post['caract'] as $caract_var ) {
                                if ( $caract_var !="" ) {
                                     $arrCaract[] = $caract_var;
                                }
                            }
                            $this->model_catalog_product->updateCaract($this->request->get['product_id'], serialize( $arrCaract ) );
                        } else {
                            $this->model_catalog_product->updateCaract($this->request->get['product_id'], '' );
                        }
                        if(isset( $this->request->post['descuento'] )) {
							$desc_var = '';
                            if ( $this->request->post['descuento'] !="" && is_numeric( $this->request->post['descuento'] )  ) {
                                     $desc_var = $this->request->post['descuento'];
                            }
                            $this->model_catalog_product->updateDesc($this->request->get['product_id'], $desc_var );
                        } else {
                            $this->model_catalog_product->updateDesc($this->request->get['product_id'], '' );
                        }
  	...
  	private function getForm() {
        ...
        if (isset( $this->request->post['cuidado']) ) {
                  ...
                  $this->data['cuidado'] = $product_info['cuidado'];
    	} else {
                  $this->data['cuidado'] = '';
    	}

        if (isset( $this->request->post['corte']) ) {
                  $this->data['corte'] = $this->request->post['corte'];
	} elseif (isset($product_info)) {
                  $this->data['corte'] = $product_info['corte'];
    	} else {
                  $this->data['corte'] = '';
    	}

        /* Talla, Subtalla, Color, Caracteristica */
	if (isset($product_info)) {
                  $this->data['talla'] = $product_info['talla'];
    	} else {
                  $this->data['talla'] = '';
    	}

	if (isset($product_info)) {
                  $this->data['subtalla'] = $product_info['subtalla'];
    	} else {
                  $this->data['subtalla'] = '';
    	}
        ...
        if (isset($product_info)) {
               if( $product_info['color'] ) {
                   $this->data['color_img'] = $this->model_tool_image->resize($product_info['color'], 100, 100);
                   $this->data['color'] = $product_info['color'];
               } else {
                   $this->data['color_img'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
                   $this->data['color'] = '';
               }
    	} else {
                  $this->data['color_img'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
	...
	if (isset($product_info)) {
                  $this->data['nombre_color'] = $product_info['nombre_color'];
    	} else {
                  $this->data['nombre_color'] = '';
    	}



	if (isset($product_info)) {
                  $this->data['caract'] = $product_info['caracteristica'];
    	} else {
                  $this->data['caract'] = '';
    	}

	if (isset($product_info)) {
                  $this->data['descuento'] = $product_info['descuento'];
    	} else {
                  $this->data['descuento'] = '';
    	}
        ...
        if (isset( $this->request->post['novedad']) ) {
                  $this->data['novedad'] = $this->request->post['novedad'];
	} elseif (isset($product_info)) {
                  $this->data['novedad'] = $product_info['novedad'];
    	} else {
                  $this->data['novedad'] = '';
    	}

        $this->data['Tallacombo'] = $this->model_catalog_product->getTallas();
        $this->data['SubTallacombo'] = $this->model_catalog_product->getsSubTallas();
